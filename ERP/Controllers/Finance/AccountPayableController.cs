...
namespace ERP.Controllers.Finance
{
    public class AccountPayableController : Controller
    {
        ...
        public async Task<IActionResult> AccountPayable(int page = 1, int pageSize = 5, string activeTab = "form")
        {
            ...
            //return View("AccountPayable",model);
        }
            ...
            try
            {
                ...
                if (jvm.JournalEntry.Id > 0)
                {
                    ...
                    if (existingEntry != null)
                    {
                        ...
                        await _context.SaveChangesAsync();
                        _notyf.Success("Account Payable Voucher Update Successfully");
                    }
                }
                else
                {
                    ...
                    await _context.SaveChangesAsync();
                    _notyf.Success("Account Payable Voucher Create Successfully");
                }
        ...
        public async Task<IActionResult> Delete(int id, int page = 1, int pageSize = 5)
        {
            ...
            if (journal != null)
            {
                ...
                await _context.SaveChangesAsync();
                _notyf.Success("AccountPayable Voucher Delete Successfully");
            }
            return RedirectToAction("AccountPayable", new { page, pageSize, activeTab = "list" });
        }
    }
